  ...
  return (
    <div className={`flex items-center gap-0.5 text-yellow-400 ${opacity}`}>
      {Array.from({ length: count }).map((_, i) => (
        <span
          key={i}
          className="drop-shadow-[0_0_10px_rgba(250,204,21,0.35)]"
        >
          ★
        </span>
      ))}
  ...
  return (
    <label className="block">
      ...
      <select
        ...
        className="w-full rounded-xl bg-black/30 border border-white/10 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-yellow-400/40"
      >
        {options.map((o) => (
          <option
            key={o.value}
...
export default function WatchlistClient({
  ...
  const states = useMemo(() => {
    return [...new Set(data.map((p) => p.state))]
      .filter((s): s is string => Boolean(s))
      .sort((a, b) => a.localeCompare(b));
  }, [data]);

  const classes = useMemo(() => {
    return [...new Set(data.map((p) => p.classYear))]
      .filter((c): c is number => typeof c === "number")
      .sort((a, b) => a - b);
  }, [data]);

  const positions = useMemo(() => {
    return [...new Set(data.map((p) => p.position))]
  ...
  const filtered = useMemo(() => {
    ...
    return data
      .filter((p) => {
        ...
        const matchStar =
          star === "all" ? true : p.stars === star;

        const matchState =
          state === "all" ? true : p.state === state;
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-14">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-sm text-gray-300 max-w-2xl">
            Filter by stars, state, class and position. Click a player to
            view their profile.
          </p>
        </div>
        ...
        <div className="mt-10 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            ...
            <div className="md:col-span-4">
              <label className="block">
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-black/30 border border-white/10 px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-yellow-400/40"
                />
              </label>
            </div>
          ...
          <div className="mt-5 flex items-center justify-between">
            ...
            <button
              onClick={reset}
              className="rounded-xl border border-white/10 bg-black/30 px-4 py-2 text-sm hover:border-yellow-400/40"
            >
              Reset
            </button>
          </div>
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {pageItems.map((p) => (
            <Link
              ...
              className="group rounded-2xl border border-white/10 bg-white/[0.03] p-5 hover:border-yellow-400/40 transition"
            >
              ...
              <p className="mt-1 text-sm text-gray-300">
                {p.height} • {p.position} • {p.classYear}
              </p>
        ...
        <div
          id="criteria"
          className="mt-16 rounded-2xl border border-white/10 bg-white/[0.03] p-7"
        >
          ...
          <p className="mt-3 text-sm text-gray-300">
            NCP evaluates players on production, tools,
            competitiveness, coachability and long-term upside—not
            hype.
          </p>
        </div>
      </div>
